        ...
        const encriptados = await Promise.all(
            files.map(async (file) => {
                ...
                return new Promise((resolve, reject) => {
                    ...
                    input.on('error', (err) => reject(`❌ Error al leer el archivo ${file.originalname}: ${err}`));
                    output.on('error', (err) => reject(`❌ Error al escribir el archivo ${file.originalname}: ${err}`));
                });
            })
        );
        res.status(200).json({ message: 'Archivos encriptados exitosamente', encriptados });
...
export const decryptFiles = async (req, res) => {
    try {
        ...
        if (!files || files.length === 0) {
            return res.status(400).json({ message: 'No se han proporcionado archivos para desencriptar' });
        }

        const decryptedFiles = await Promise.all(
            files.map(file => new Promise((resolve, reject) => {
                // Definir rutas del archivo encriptado y desencriptado
                const encryptedFilePath = path.join(process.cwd(), 'encrypted-files', file.trim());
                const decryptedFilePath = path.join(process.cwd(), 'decrypted-files', `decrypted-${file.replace('.enc', '').trim()}`);
                ...
                input.on('data', (chunk) => {
                    if (isFirstChunk) {
                        ...
                        console.log(`🔑 IV leído para ${file} y desencriptación iniciada`);
                        ...
                        output.on('finish', () => {
                            console.log(`✅ Archivo ${file} desencriptado exitosamente`);
                            resolve(path.basename(decryptedFilePath));
                        });

                        output.on('error', (err) => {
                            console.error(`❌ Error al escribir el archivo desencriptado para ${file}:`, err);
                            reject(err);
                        });
                    } else {
                        chunks.push(chunk);
                ...
                input.on('error', (err) => {
                    console.error(`❌ Error al leer el archivo encriptado para ${file}:`, err);
                    reject(err);
                });
            }))
        );
